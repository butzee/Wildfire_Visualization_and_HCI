...
const buttonSQL = document.getElementById('queryButton2');
let yearsArray;
...
buttonSQL.addEventListener('click', async () => {
    ...
    try {
        yearsArray = await window.electronAPI.getAllFires(causeOptions, sizeOptions);
        console.log("done");
    } catch (error) {
        console.error(error);
    }
...
function updateScatter(value) {
    console.log(yearsArray[value][0].FIRE_SIZE)
